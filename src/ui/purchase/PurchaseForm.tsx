...
export default function PurchaseForm({amount_cents}: { amount_cents: number }) {
    ...
    const invoiceDataMethods = useForm<purchaseInvoiceDataFields>({resolver: zodResolver(purchaseInvoiceDataModel), mode: "all"})
    ...
    return (
        <div className="flex flex-col items-center mt-2">
            ...
            <FormProgressIndicator currentStep={step} steps={["Email", "Invoice data", "Payment", purchaseResult?.success !== false? "Finished": "Error"]} className="w-full mb-3"/>
            {stepContent}
            <div className="flex gap-2 justify-between w-full mt-4">
                <div>
                    { step === PurchaseStep.EMAIL &&  <Button onPress={handleReturnToCart} color="primary">Return to cart</Button>}
                    { step !== PurchaseStep.EMAIL && step !== PurchaseStep.FINISHED &&  <Button onPress={handleNavigatePrevious} color="primary">Previous</Button>}
                </div>
                {step !== PurchaseStep.PAYMENT && step !== PurchaseStep.FINISHED && <Button onPress={handleNavigateNext} color="primary">Next</Button>}
                {step === PurchaseStep.PAYMENT && <Button onPress={handleFinish} color="secondary">Finish purchase</Button>}
            </div>
            <DevTool control={emailMethods.control} placement="top-right"/>
            </div>
        </div>
